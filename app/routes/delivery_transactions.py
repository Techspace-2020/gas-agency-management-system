    ...
    try:
        ...
        if request.method == "POST":
            ...
            if "reset_db" in request.form:
                ...
                db.execute(text("DELETE FROM office_counter_sales WHERE stock_day_id = :s_id"), {"s_id": s_id})
                ...
                db.commit()
                flash("All transaction records cleared successfully.", "info")
                return redirect(url_for("delivery_transactions.transactions_view"))
            ...
            if no_mov_checked == 1:
                ...
                for (b_id, t_id), q in data_map.items():
                    if t_id and (q['r'] > 0 or q['n'] > 0 or q['d'] > 0 or q['tv'] > 0):
                        ...
                        # --- 3. SYNC WITH OFFICE COUNTER SALES (OFFICE ID 11) ---
                        if str(b_id) == "11":
                            # OUTER COALESCE added to handle NOT NULL constraints for first-time entries
                            db.execute(text("""
                                INSERT INTO office_counter_sales (
                                    stock_day_id, cylinder_type_id, 
                                    opening_refill, received_refill, sold_refill,
                                    opening_nc, received_nc, sold_nc,
                                    opening_dbc, received_dbc, sold_dbc
                                )
                                VALUES (
                                    :s_id, :t_id, 
                                    COALESCE((SELECT val FROM (SELECT COALESCE(closing_refill, 0) as val FROM office_counter_sales WHERE cylinder_type_id = :t_id AND stock_day_id < :s_id ORDER BY stock_day_id DESC LIMIT 1) as t1), 0),
                                    :r, 0,
                                    COALESCE((SELECT val FROM (SELECT COALESCE(closing_nc, 0) as val FROM office_counter_sales WHERE cylinder_type_id = :t_id AND stock_day_id < :s_id ORDER BY stock_day_id DESC LIMIT 1) as t2), 0),
                                    :n, 0,
                                    COALESCE((SELECT val FROM (SELECT COALESCE(closing_dbc, 0) as val FROM office_counter_sales WHERE cylinder_type_id = :t_id AND stock_day_id < :s_id ORDER BY stock_day_id DESC LIMIT 1) as t3), 0),
                                    :d, 0
                                )
                                ON DUPLICATE KEY UPDATE 
                                    received_refill = :r,
...
def download_delivery_log(day_id):
    ...
    try:
        day_info = db.execute(text("SELECT stock_date FROM stock_days WHERE stock_day_id = :id"), {"id": day_id}).fetchone()
